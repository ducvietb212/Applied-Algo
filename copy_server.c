...
void addNewUser(char username[], char user_password[], char email[]){
    ...
    if(strcmp(email, "") == 0){
        sprintf(query, "INSERT INTO User_info(username, password, role) VALUES ('%s', '%s', 0);", username, user_password);
    }
    else{
        sprintf(query, "INSERT INTO User_info(username, password, role) VALUES ('%s', '%s', '%s', 0);", username, user_password, email);
    }
...
//     strcpy((exam_list)[i].title, row[1]);
//     (exam_list)[i].number_of_question = atoi(row[2]);
//     (exam_list)[i].admin_id = atoi(row[3]);
//     //check exam_list
//     printf("Exam %d: %s\n", j+1, (*exam_list)[j].title);
    
//     //Send exam list to client
//     char rcv[10];
//     send(socket, ((*exam_list)+j), sizeof(exam_data), 0);
//     recv(socket, rcv, sizeof(rcv), 0);
...
void addTestFromBank(int socket, char client_message[]){
    ...
    {
        sprintf(query, "SELECT COUNT(*) FROM Test_bank;");
        res = make_query(query);
        row = mysql_fetch_row(res);
        ...
        sprintf(query, "SELECT * FROM Test_bank;");
        res = make_query(query);
        ...
        for(int i=0; i<num_of_test_in_bank; i++){
            ...
            strcpy(test_info.test_subject, row[2]);
            test_info.num_of_question = atoi(row[3]);
...
void createTest(int socket, char client_message[]){
    ...
    //send number of subject
    {
        sprintf(query, "SELECT COUNT(DISTINCT(question_subject)) FROM Question;");
        res = make_query(query);
        row = mysql_fetch_row(res);
        num_of_suject = atoi(row[0]);
        mysql_free_result(res);
        send(socket, &num_of_suject, sizeof(int), 0);
        recv(socket, oke_signal, OKE_SIGNAL_LEN, 0);
    }

    // a loop to transfer subject to admin to choose
    {   
        transfer_subject subject_data;
        sprintf(query, "SELECT question_subject, count(*) FROM Question GROUP BY question_subject;");
        res = make_query(query);

        while( (row = mysql_fetch_row(res)) != NULL){
            memset(&subject_data, 0, sizeof(transfer_subject));
            strcpy(subject_data.subject, row[0]);
            subject_data.max_num_of_question = atoi(row[1]);
            send(socket, &subject_data, sizeof(transfer_subject), 0);
            recv(socket, oke_signal, OKE_SIGNAL_LEN, 0);
        }
    ...
    //check flags to see if admin want to continue
    {
        recv(socket, &flag, sizeof(int), 0);
        send(socket, oke_signal, strlen(oke_signal) + 1, 0);
        if(flag == 0){
            return;
        }
    }
    ...
    //check test_name to make sure no duplicate
    {
        ...
        sprintf(query, "SELECT * FROM Test_bank WHERE test_name = '%s';", test_name);
        res = make_query(query);
        row = mysql_fetch_row(res);
        if(row == NULL){
            flag = 1;
    ...
    //receive the new_test info
    memset(&new_test, 0, sizeof(create_test));
    recv(socket, &new_test, sizeof(create_test), 0);
    
    //make query to create new test
    {   
        char list_question_id[new_test.num_of_question][51];
        
        sprintf(query, "INSERT INTO Test_bank VALUES ('%s', '%s', %d);", new_test.test_name, new_test.test_subject, new_test.num_of_question);
        make_query(query);

        sprintf(query, "SELECT question_id FROM Question WHERE question_subject = '%s' ORDER BY RAND() LIMIT %d;", new_test.test_subject, new_test.num_of_question);
        res = make_query(query);

        for(int i=0; i<new_test.num_of_question; i++){
            row = mysql_fetch_row(res);
            strcpy(list_question_id[i], row[0]);
        }

        int test_id;
        sprintf(query, "SELECT test_id FROM Test_bank WHERE test_name = '%s';", new_test.test_name);
        res = make_query(query);
        row = mysql_fetch_row(res);
        test_id = atoi(row[0]);
        mysql_free_result(res);
...
void addTestToRoom(int socket, char client_message[]){
    add_test_room *request = (add_test_room*)client_message;
    ...
    make_query(query);
    send(socket, "OK", sizeof("OK"), 0); // Send successfull message to client.
}
...
void getExam(int socket, char client_message[]){
    char cur[20];
    srand(time(NULL));
...
void User_showSubmittedRecordOfTest(int socket, char client_message[]){
    ...
    //receive username
    {
        ...
        recv(socket, username, sizeof(int), 0);
    }
...
void connection_handler(int socket){
    ...
    case 203:
        ...
        break;
    case 204:
        requestQuestionList(socket, client_message);
        break;
    case 205:
    ...
    case 813:
        addTestFromBank(socket, client_message);
        break;
    case 814:
        createTest(socket, client_message);
        break;
    case 815:
        deleteTestFromRoom(socket, client_message);
        break;
    case 816:
        Admin_showSubmittedRecordOfTest(socket, client_message);
        break;
    case 817:
        Admin_showAllSubmittedRecord(socket, client_message);
        break;
    case 831:
        User_showSubmittedRecordOfTest(socket, client_message);
        break;
    case 832:
        User_showAllSubmittedRecord(socket, client_message);
        break;
